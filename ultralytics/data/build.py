...
from .dataset import YOLODataset,MOVEDETDataset,MOVEHomoDETDataset,MOVEHomoDETDataset_stream
...
def build_dataloader(dataset, batch, workers, shuffle=True, rank=-1):
    ...
    # sampler = None if rank == -1 else DistributedSampler(StreamSampler(dataset, batch, p=0.95, seed=1))
...
def check_source(source):
    ...
    if isinstance(source, (str, int, Path)):  # int for local usb camera
        ...
        tensor = True
    elif isinstance(source, dict) and "backbone" in source:
        in_memory = True
    else:
