...
class MSTF_STREAM2(nn.Module): #
    def __init__(self, in_channels, epoch_train=0, depth=1.0,
        ...
        iter_max Number of iterations
        '''
        super(MSTF_STREAM2, self).__init__()
        self.in_channels = in_channels
        self.epoch_train = epoch_train
        # buffer
        self.buffer = StreamBuffer("MemoryAtten", number_feature=n_levels)
    ...
    def forward(self, x):
        ...
        if fmaps_new[0].device.type == "cpu" or (img_metas[0]["epoch"] < self.epoch_train and self.training):
            ...
            pan_out2 = torch.cat([self.jian2(rurrent_pan_out2), self.jian2(support_pan_out2)], dim=1) + rurrent_pan_out2 + rurrent_x2
            pan_out1 = torch.cat([self.jian1(rurrent_pan_out1), self.jian1(support_pan_out1)], dim=1) + rurrent_pan_out1 + rurrent_x1
            pan_out0 = torch.cat([self.jian0(rurrent_pan_out0), self.jian0(support_pan_out0)], dim=1) + rurrent_pan_out0 + rurrent_x0
        
        return [pan_out2, pan_out1, pan_out0]
class MSTF_STREAM3(nn.Module): #
    def __init__(self, in_channels, epoch_train=0, depth=1.0,
        width=1.0, n_levels=3, depthwise=False,
        act=nn.SiLU(),):
        '''
        ...
        iter_max Number of iterations
        '''
        super(MSTF_STREAM3, self).__init__()
        
        conv = DWConv if depthwise else Conv
        self.convs1 = nn.ModuleList([conv(in_channels[i], in_channels[i], 1) 
                                    for i in range(n_levels)])
        
        self.convs2 = nn.ModuleList([conv(in_channels[i], in_channels[i], 1) 
                                    for i in range(n_levels)])  # optional act=FReLU(c2)
        
        self.in_channels = [i//2 for i in in_channels]
        self.epoch_train = epoch_train
        # buffer
        self.buffer = StreamBuffer("MemoryAtten", number_feature=n_levels)
        ...
        # bottom-up conv
        self.bu_conv2 = conv(
            int(self.in_channels[0] * width), int(self.in_channels[0] * width), 3, 2, act=act
        )
        self.C3_n3 = C3_wise(
            int(2 * self.in_channels[0] * width),
            int(self.in_channels[1] * width),
        ...
        # bottom-up conv
        self.bu_conv1 = conv(
            int(self.in_channels[1] * width), int(self.in_channels[1] * width), 3, 2, act=act
        )
        self.C3_n4 = C3_wise(
            int(2 * self.in_channels[1] * width),
        ...
        self.jian2 = conv(
                    int(self.in_channels[0] * width),
                    int(self.in_channels[0] * width) // 2,
                    k=1,
                    s=1,
                    act=act,
                )
        ...
        self.jian1 = conv(
            int(self.in_channels[1] * width),
            int(self.in_channels[1] * width) // 2,
                    k=1,
                    s=1,
                    act=act,
                )
        ...
        self.jian0 = conv(
            int(self.in_channels[2] * width),
            int(self.in_channels[2] * width) // 2,
                    k=1,
                    s=1,
                    act=act,
                )
    ...
    def forward(self, x):
        ...
        for i in range(len(x[1:])):
            ...
            out, fmap = self.convs1[i](x[1:][i]).chunk(2, 1) #128,128
            # fmap = self.convs1[i](fmap) #hidden
            out_new.append(out)
            fmaps_new.append(fmap)
        ...
        if fmaps_new[0].device.type == "cpu" or (img_metas[0]["epoch"] < self.epoch_train and self.training):
            pan_out2 = torch.cat([self.jian2(rurrent_pan_out2), self.jian2(support_pan_out2)], dim=1) + rurrent_x2
            pan_out2 = self.convs2[0](torch.cat([out_new[0],pan_out2], 1)) + x[1:][0]
            
            pan_out1 = torch.cat([self.jian1(rurrent_pan_out1), self.jian1(support_pan_out1)], dim=1) + rurrent_x1
            pan_out1 = self.convs2[1](torch.cat([out_new[1],pan_out1], 1)) + x[1:][1]
            
            pan_out0 = torch.cat([self.jian0(rurrent_pan_out0), self.jian0(support_pan_out0)], dim=1) + rurrent_x0
            pan_out0 = self.convs2[2](torch.cat([out_new[2],pan_out0], 1)) + x[1:][2]
