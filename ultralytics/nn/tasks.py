...
from ultralytics.utils.loss import v8ClassificationLoss, MOVEDetectionLoss,v8DetectionLoss, v8PoseLoss, v8SegmentationLoss
from ultralytics.utils.plotting import feature_visualization
from ultralytics.utils.torch_utils import (fuse_conv_and_bn, fuse_deconv_and_bn, initialize_weights,
                                                intersect_dicts, intersect_dicts_by_model, make_divisible, model_info, scale_img, time_sync)
...
class BaseModel(nn.Module):
    ...
    def _predict_once(self, x, profile=False, visualize=False):
        ...
        for m in self.model:
            ...
            try:
                ...
                print(f"error model number: {m.i}")
                raise
    ...
    def load(self, weights, verbose=True):
        ...
        # if fintinue_model:#Only a small number of parameters were adjusted
        #     csd = intersect_dicts_by_fintinue(csd, self.state_dict())  # As long as the modules are the same, the ones with different shapes will be clipped and the insufficient ones initialized
        # else:
        csd = intersect_dicts_by_model(csd, self.state_dict())  # intersect
        # csd = intersect_dicts(csd, self.state_dict())  # intersect
        # csd = {key:value for key, value in csd.items() if int(key.split(".")[1]) <= 9}
...
class MOVEDetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolov8n.yaml', ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        self.model, self.save = parse_motion_model(deepcopy(self.yaml), ch=ch, verbose=verbose)  # model, savelist
...
def load_weight_as_name(weight,model,device=None):
...
def attempt_load_one_weight(weight, device=None, inplace=True, fuse=False):
    ...
    if 'model' in ckpt:
        ...
        LOGGER.error(f"pretrain modoule is not yolov8")
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        #     m_ = CONVERTERS.build(args)
        elif m in (Block,BlockE1):
            c2 = args[3]
        else:
            c2 = ch[f]
...
def parse_motion_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for key,value in d.items():
        if key in ['backbone', 'motion', 'head']:
            net += value
    for i, (f, n, m, args) in enumerate(net):  # from, number, module, args
        m = getattr(torch.nn, m[3:]) if 'nn.' in m else globals()[m]  # get module
        for j, a in enumerate(args):
            if isinstance(a, str):
                with contextlib.suppress(ValueError):
        ...
        if m in (Classify, Conv,DCNV3_conv, ConvTranspose, GhostConv, Bottleneck,Bottleneck_DCNV3, GhostBottleneck, SPP, SPPF, SPPELAN, DWConv, Focus,
            ...
            c2 = ch[f][args[0]]
        elif m in (DetectMOVE,Detect, Segment, Pose):
            args.append([ch[x] for x in f])
            if m is Segment:
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)
        elif m is RTDETRDecoder:  # special case, channels arg must be passed in index 1
        ...
        #     m_ = CONVERTERS.build(args)
            ...
            c2 = m.output_dict[args[-1]]
                ...
                assert args[-1] in output_dict, LOGGER.error(f"Homograph not key:{args[-1]} in this output_dict")
                c2 = output_dict[args[-1]]
        elif m is TemporalShift:
            c1, c2 = ch[f], ch[f]
            args.insert(1, c1)
        elif m is MutiFreatureMemory:
            ...
            args.insert(0, c1)
                   ...
                   MSTF, MSTF_singal_flow,VelocityNet_baseline3_split_dim,VelocityNet_baseline3_iter,MSTF_STREAM,MSTF_STREAM2,MSTF_STREAM3):
...
def guess_model_task(model):
    ...
    if isinstance(model, nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, Detect) or isinstance(m, DetectMOVE):
                return 'detect'
