...
def non_max_suppression_moving(
        prediction,
        conf_thres=0.25,
        iou_thres=0.45,
        moving_threshold = 0.25,  # 替换为您的阈值
        classes=None,
        agnostic=False,
        multi_label=False,
        labels=(),
    ...
    Returns:
        (List[torch.Tensor]): A list of length batch_size, where each element is a tensor of
            shape (num_boxes, 6 + 1 + num_masks) containing the kept boxes, with columns
            (x1, y1, x2, y2, confidence, class, is_moving, mask1, mask2, ...).
    """
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # Detections matrix nx6 (xyxy, conf, cls)
        box, cls, is_moving, mask = x.split((4, nc, 1, nm), 1)
        ...
        if multi_label:
            i, j = torch.where(cls > conf_thres)
            x = torch.cat((box[i], x[i, 4 + j, None], j[:, None].float(), is_moving[i], mask[i]), 1)
        else:  # best class only
            conf, j = cls.max(1, keepdim=True)
            x = torch.cat((box, conf, j.float(), is_moving, mask), 1)[conf.view(-1) > conf_thres]
    ...
    #         output_[:, 6] = torch.where(output_[:, 6] >= moving_threshold, 1, 0)
    return output
