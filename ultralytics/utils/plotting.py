...
def plot_images(images,
    ...
    max_size = 1920*4  # max image size
...
@threaded
# delete draw text by guojiahao
def plot_videos(video_batch_list,
                batch_list_idx,
                cls_list,
                bboxes_list,
                masks=np.zeros(0, dtype=np.uint8),
    ...
    for k, images in enumerate(video_batch_list):
        ...
        # Annotate
        fs = int((h + w) * ns * 0.01)  # font size
        annotator = Annotator(mosaic, line_width=1, font_size=3, pil=True, example=names)
        for i in range(i + 1):
            x, y = int(w * (i // ns)), int(h * (i % ns))  # block origin
            annotator.rectangle([x, y, x + w, y + h], None, (255, 255, 255), width=2)  # borders
            if paths:
                annotator.text((x + 5, y + 5), text=Path(paths[i]).name[:40], txt_color=(220, 220, 220))  # filenames
            annotator.text((x + 5, y + 5), text=str(len(bboxes)), txt_color=(220, 220, 220))
            if len(cls) > 0:
                idx = batch_idx == i
                classes = cls[idx].astype('int')
